...
line_hin star_hin;
line_trainer trainer_ap, trainer_pp, trainer_vp, trainer_wp, trainer_yp;
...
void *TrainModelThread(void *id) 
{
	...
	while (1)
	{
		...
		if(strstr(attribute_types, "a")) trainer_ap.train_sample(alpha, error_vec, func_rand_num, next_random);
		if(strstr(attribute_types, "p")) trainer_pp.train_sample(alpha, error_vec, func_rand_num, next_random);
		if(strstr(attribute_types, "v")) trainer_vp.train_sample(alpha, error_vec, func_rand_num, next_random);
		if(strstr(attribute_types, "w")) trainer_wp.train_sample(alpha, error_vec, func_rand_num, next_random);
		if(strstr(attribute_types, "y")) trainer_yp.train_sample(alpha, error_vec, func_rand_num, next_random);
...
void TrainModel() {
	...
	if(strstr(attribute_types, "a")) trainer_ap.init('a', &star_hin, negative);
	if(strstr(attribute_types, "p")) trainer_pp.init('p', &star_hin, negative);
	if(strstr(attribute_types, "v")) trainer_vp.init('v', &star_hin, negative);
	if(strstr(attribute_types, "w")) trainer_wp.init('w', &star_hin, negative);
	if(strstr(attribute_types, "y")) trainer_yp.init('y', &star_hin, negative);
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
		...
		printf("\nExamples:\n");
		printf("./emb_dblp -attribute attribute.txt -center center.txt -hin hin.txt -output attribute.emb -output-center center.emb -types apvwy -size 20 -negative 5 -samples 100\n\n");
		return 0;
	}
	output_file[0] = 0;
